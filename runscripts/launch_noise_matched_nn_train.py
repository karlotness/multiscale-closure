    ...
    for name, spec_str, gen_mode in [("net-roll", "", "network-noise-onestep"), ("gaussian", spec_str, "schedule-only")]:
        ...
        for orig_net in ORIG_NETS:
            ...
            for repeat in range(NUM_REPEATS):
                ...
                # Launch job
                job_id = sbatch_launch(["run-noise-match-net-roll-train.sh", str(run_dir), str(TRAIN_FILE.parent), str(VAL_FILE.parent), str(LIVE_DATA_FILE.parent), str(orig_net), gen_mode, spec_str, str(num_steps), str(NUM_SNAPSHOTS)])
