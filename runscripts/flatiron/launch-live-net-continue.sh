...
readonly OUT_DIR="${SCRATCH}/closure/run_outputs/continue-live-data-runs-${LAUNCH_TIME}"
...
for noise_mode in 'noiseless' 'add-noise'; do
    for net_file in "${NET_FILES[@]}"; do
        for num_candidates in '20' '50'; do
            if [[ $net_file =~ /closure/run_outputs/run-varied-data-size-([[:digit:]]+-[[:digit:]]+)/([^/]+)/size([[:digit:]]+)-scale([[:digit:]]+)/([^/]+)/weights/([^/]+).eqx ]]; then
                ...
                for lr_mode in "${LR_MODES[@]}"; do
                    ...
                    for repeat in $(seq "$NUM_REPEATS" ); do
                        ...
                        run_out=$(echoing_sbatch continue-train-live-data-randfactor.sh "$launch_dir" "$TRAIN_DIR" "$VAL_DIR" "$continue_scale" "$net_file" "$lr_mode" "$num_candidates" "$NUM_WINNERS" "$noise_mode")
