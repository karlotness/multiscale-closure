...
readonly OUT_DIR="${SCRATCH}/closure/run_outputs/continue-runs-noisy-${LAUNCH_TIME}"
    ...
    if [[ $net_file =~ /closure/run_outputs/run-varied-data-size-([[:digit:]]+-[[:digit:]]+)/([^/]+)/size([[:digit:]]+)-scale([[:digit:]]+)/([^/]+)/weights/([^/]+).eqx ]]; then
        ...
        for lr_mode in "${LR_MODES[@]}"; do
            ...
            for repeat in $(seq "$NUM_REPEATS" ); do
                ...
                run_out=$(echoing_sbatch continue-noisy-train-randfactor.sh "$launch_dir" "$TRAIN_DIR" "$VAL_DIR" "$continue_scale" "$net_file" "$lr_mode")
