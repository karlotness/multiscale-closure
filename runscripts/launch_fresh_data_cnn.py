...
OUT_DIR=SCRATCH / "closure" / "run_outputs"/ f"fresh-data-matched-cnn-{LAUNCH_TIME}"
ORIG_NETS = [
...
for fresh_len, (num_steps, _) in itertools.product(FRESHNESS_LENGTHS, STEP_LAYER_VARS):
    spec_str = ""
    name = "net-roll"
    gen_mode = "network-noise-onestep"
    mode_dir = OUT_DIR / f"fresh{fresh_len:04}" / f"steps{num_steps:04}" / name
    eval_weights = []
    eval_job_ids = []
    num_snapshots = round((100 * 87) / fresh_len)
    for orig_net in ORIG_NETS:
        ...
        for repeat in range(NUM_REPEATS):
            ...
            # Launch job
            job_id = sbatch_launch(["run-noise-match-net-roll-train.sh", str(run_dir), str(TRAIN_FILE.parent), str(VAL_FILE.parent), str(LIVE_DATA_FILE.parent), str(orig_net), gen_mode, spec_str, str(num_steps), str(num_snapshots), "gz-fcnn-v1", str(fresh_len)])
