...
for expansion_level in '0.05' '0.10' '0.20' '0.50' '0.75'; do
    ...
    # Train
    TRAIN_OUT1=$(sbatch --wrap="singularity exec --nv '${SCRATCH}/closure/closure.sif' python generate_data.py '${SCRATCH}/closure/${DATA_DIR}/train/' qg 0 --config ${data_config} --num_trajs=275 --traj_slice '0:60' --coarse_op ${COARSEN_OP} --subsample 1000 --small_size 64 48 --dt 3600.0 --tmax 470160000.0 --twarmup 155520000.0" --job-name="qg-gen-train" --time="4:00:00" --cpus-per-task=1 --mem="20G" --gpus=1 --partition=gpu)
    TRAIN_JOBID1=$(get_job_id "$TRAIN_OUT1")
    sleep 0.5
    TRAIN_OUT2=$(sbatch --wrap="singularity exec --nv '${SCRATCH}/closure/closure.sif' python generate_data.py '${SCRATCH}/closure/${DATA_DIR}/train/' qg 0 --config ${data_config} --num_trajs=275 --traj_slice '60:120' --coarse_op ${COARSEN_OP} --subsample 1000 --small_size 64 48 --dt 3600.0 --tmax 470160000.0 --twarmup 155520000.0" --job-name="qg-gen-train" --time="4:00:00" --cpus-per-task=1 --mem="20G" --gpus=1 --partition=gpu)
    TRAIN_JOBID2=$(get_job_id "$TRAIN_OUT2")
    sleep 0.5
    TRAIN_OUT3=$(sbatch --wrap="singularity exec --nv '${SCRATCH}/closure/closure.sif' python generate_data.py '${SCRATCH}/closure/${DATA_DIR}/train/' qg 0 --config ${data_config} --num_trajs=275 --traj_slice '120:180' --coarse_op ${COARSEN_OP} --subsample 1000 --small_size 64 48 --dt 3600.0 --tmax 470160000.0 --twarmup 155520000.0" --job-name="qg-gen-train" --time="4:00:00" --cpus-per-task=1 --mem="20G" --gpus=1 --partition=gpu)
    TRAIN_JOBID3=$(get_job_id "$TRAIN_OUT3")
    sleep 0.5
    TRAIN_OUT4=$(sbatch --wrap="singularity exec --nv '${SCRATCH}/closure/closure.sif' python generate_data.py '${SCRATCH}/closure/${DATA_DIR}/train/' qg 0 --config ${data_config} --num_trajs=275 --traj_slice '180:240' --coarse_op ${COARSEN_OP} --subsample 1000 --small_size 64 48 --dt 3600.0 --tmax 470160000.0 --twarmup 155520000.0" --job-name="qg-gen-train" --time="4:00:00" --cpus-per-task=1 --mem="20G" --gpus=1 --partition=gpu)
    TRAIN_JOBID4=$(get_job_id "$TRAIN_OUT4")
    sleep 0.5
    TRAIN_OUT5=$(sbatch --wrap="singularity exec --nv '${SCRATCH}/closure/closure.sif' python generate_data.py '${SCRATCH}/closure/${DATA_DIR}/train/' qg 0 --config ${data_config} --num_trajs=275 --traj_slice '240:' --coarse_op ${COARSEN_OP} --subsample 1000 --small_size 64 48 --dt 3600.0 --tmax 470160000.0 --twarmup 155520000.0" --job-name="qg-gen-train" --time="4:00:00" --cpus-per-task=1 --mem="20G" --gpus=1 --partition=gpu)
    TRAIN_JOBID5=$(get_job_id "$TRAIN_OUT5")
    sleep 0.5
    TRAIN_OUT=$(sbatch --wrap="singularity exec '${SCRATCH}/closure/closure.sif' python generate_data.py '${SCRATCH}/closure/${DATA_DIR}/train/${COARSEN_OP}/' combine_qg_slice" --job-name="qg-combine" --time="8:00:00" --cpus-per-task=2 --mem="20G" --dependency="afterok:$TRAIN_JOBID1:$TRAIN_JOBID2:$TRAIN_JOBID3:$TRAIN_JOBID4:$TRAIN_JOBID5" --kill-on-invalid-dep=yes)
    sleep 0.5
    # Val
