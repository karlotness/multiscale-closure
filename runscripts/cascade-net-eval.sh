...
#SBATCH --job-name=cascade-net-eval
#SBATCH --time=1:30:00
...
if [[ $# -lt 2 ]]; then
    ...
    echo "Usage cascade-net-eval.sh EVAL_TYPE NET_DIR"
    exit 1
fi
...
singularity run --nv "$SINGULARITY_CONTAINER" \
            python "${CHECKOUT_DIR}/src/cascaded_eval.py" "$NET_DIR" "$EVAL_DATA_DIR" "$EVAL_TYPE" \
            --seed=0 \
            --sample_seed=0 \
            --num_samples=1024
